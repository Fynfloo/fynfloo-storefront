...
export function ProductCard({ product, currency, priority = false }: ProductCardProps) {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-[var(--radius-button)] bg-gray-100 aspect-[3/4]">
        {primaryImage ? (
          <Image
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {product.compareAtPrice && product.compareAtPrice > product.price && (
            <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium uppercase tracking-wide bg-[var(--colour-secondary)] text-white rounded-[var(--radius-button)]">
              Sale
            </span>
          )}
      ...
      <div className="mt-3 space-y-1">
        <h3 className="text-sm font-medium text-[var(--colour-primary)] group-hover:opacity-70 transition-opacity duration-200">
          {product.title}
        </h3>
