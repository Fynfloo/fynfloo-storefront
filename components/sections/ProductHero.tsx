...
export function ProductHero({ data, product, slug }: ProductHeroProps) {
  ...
  async function handleAddToCart() {
    setAdding(true);
    try {
      const { getCartToken } = await import('@/lib/cart');
  ...
  return (
    <section className="py-12 md:py-16 bg-[var(--colour-bg,#ffffff)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {showBreadcrumbs && (
          <nav
            className="mb-8 flex items-center gap-2 text-xs text-[var(--colour-primary)]
            opacity-50"
          >
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:gap-16">
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 w-16 h-16 overflow-hidden
                      rounded-[var(--radius-button)] bg-gray-100 transition-opacity
                      ${
                        i === selectedImage
                          ? 'ring-2 ring-[var(--colour-secondary)]'
          ...
          <div className="flex flex-col space-y-6">
            ...
            {isOutOfStock && (
              <div
                className="py-4 text-center border border-[var(--colour-primary)]
                border-opacity-20 rounded-[var(--radius-button)]"
              >
